...
mysql -h "$db_host" -u "$db_user" -p"$db_password" "$db_name" < billeterie.sql
if [ $? -ne 0 ]; then
    echo "Failed to import billeterie.sql into the database $db_name."
    exit 1
fi
...
echo "Setup complete. Database and user have been created, billeterie.sql has been imported, and db_config.json has been updated."
